"""Базовый OpenAI-совместимый HTTP-провайдер LLM."""
...
def fetch_models(base_url: str, api_key: str, timeout: float = 30.0) -> list[str]:
    ...
    url = f"{root}/models"
    data = request_json("GET", url, api_key, timeout=timeout)
    models: list[str] = []
...
def chat_completion(
    ...
    *,
    ...
    data = request_json("POST", url, api_key, body=payload, timeout=timeout)
